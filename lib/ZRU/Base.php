...
class Paginator
{
    ...
    public function __construct($payload, $objectItemClass, $resource)
    {
        if (is_object($payload) || is_array($payload)) {
            $this->initializeFromObject($payload, $objectItemClass, $resource);
        } elseif (is_array($payload) && array_keys($payload) === range(0, count($payload) - 1)) {
            $this->initializeFromArray($payload, $objectItemClass, $resource);
        } else {
            throw new \InvalidArgumentException('Payload must be an object or an indexed array');
        }
    }

    private function initializeFromObject($payload, $objectItemClass, $resource)
    {
    ...
    private function initializeFromArray($payload, $objectItemClass, $resource)
    {
        $this->count = count($payload);
        $this->previous = null;
        $this->next = null;
...
{
    const PAGINATOR_CLASS = 'ZRU\Paginator';
